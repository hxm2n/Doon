...
6. 새 Chrome 창에서 한컴독스로 이동해 문서를 작성하고 저장한다.
7. Doon이 결과를 검증하고 완료 상태를 사용자에게 알린다.
...
| AI 모델 | Gemini 3.5 Flash Computer Use | 화면 기반 다음 행동 제안, 의도 해석, 문서 초안 생성 |
...
앱과 목적지는 코드에 직접 박지 않는다. `TargetResolver`, `DiscordAdapter`, `BrowserAdapter` 계약 뒤에 MVP 구현을 둔다. 이후 임의 채널 검색이나 다른 브라우저를 지원할 때 구현체를 추가할 수 있어야 한다.
...
6. Chrome 기존 프로필의 새 창을 안정적으로 식별하고 한컴독스 입력을 수행할 수 있는지 확인한다.
