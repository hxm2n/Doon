...
파일은 destination 디렉터리 안의 무작위 임시 이름에 배타적으로 생성해 복사, `fsync`, 형식 검증을 마친 뒤 macOS `renameatx_np(..., RENAME_EXCL)`로 최종 이름에 원자적으로 이동한다. MVP는 기존 파일을 덮어쓰지 않는다. 같은 이름이 존재하거나 rename 시점에 충돌이 생기면 기존 파일과 임시 파일을 그대로 두지 않고 임시 파일만 제거한 뒤, 사용자에게 새 파일명 또는 다른 폴더를 선택하도록 요청한다. 이 선택으로 새 `OutputDirectoryHandle` 또는 예상 파일명을 확정한 다음 처음부터 이동을 다시 수행한다.
...
- 사용자 입력 또는 명시적 일시정지: `executing` 또는 `revising`에서 `paused_by_user`로 전환하고, 명시적 재개 시 중단 전 상태로 복귀
