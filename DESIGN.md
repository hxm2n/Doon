...
| `.bridge-panel` / `.target-card` / `.accessibility-summary` / `.window-capture-summary` / `.action-proposal-summary` | Developer-facing native spike surface using the same glass shell, compact target rows, and concise AX/capture/AI proposal result summaries | Status data must be scannable; do not turn diagnostics into decorative cards |
