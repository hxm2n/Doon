...
| `.bridge-panel` / `.target-card` / `.accessibility-summary` / `.window-capture-summary` | Developer-facing native spike surface using the same glass shell, compact target rows, and concise AX/capture result summaries | Status data must be scannable; do not turn diagnostics into decorative cards |
