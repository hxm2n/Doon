...
| `.bridge-panel` / `.target-card` / `.accessibility-summary` | Developer-facing native spike surface using the same glass shell, compact target rows, and one concise AX result summary | Status data must be scannable; do not turn diagnostics into decorative cards |
